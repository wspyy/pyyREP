...
function PlayPicWidgetFun(pictures, option) {
    ...
    if (option) {
        ...
        widgetParam.isExtentToPic = option.isExtentToPic;
        widgetParam.widgetParam = option.widgetParam;
        this.widgetClosed = option.widgetClosed;
...
//点缓冲查询
function PointBufferWidgetFun(bufferPoint, bufferRadius, searchGraphics, geomtryServerUrl, onResult,option) {
    this.type = "PointBufferWidget";
    this.id = this.type + "_" + getIdNum();
    this.isWidget = true;
    this.widgetId = "PointBufferWidget";
    this.loadInterface = "PointBufferWidget_Loaded";
    var widgetParam = new Object();
    this.widgetParam = widgetParam;
    widgetParam.bufferPoint = bufferPoint;
    widgetParam.bufferRadius = bufferRadius;
    widgetParam.geomtryServerUrl = geomtryServerUrl;

    var geometryArr = new Array();
    for (var j = 0; j < searchGraphics.length; j++) {
        if (searchGraphics[j].type.split("_")[0] == "graphic") {
            geometryArr.push(getGraphicObj(searchGraphics[j], ""));
        }
        else {
            geometryArr.push(searchGraphics[j]);
        }
    }
    widgetParam.searchGraphics = geometryArr;
    
    if (option) {
        widgetParam.widgetParam = option.widgetParam;
        widgetParam.startAngle = option.startAngle;
        widgetParam.endAngle = option.endAngle;
        widgetParam.isShowBuffer = option.isShowBuffer;
        widgetParam.bufferSymbol = option.bufferSymbol;
        this.onFault = option.onFault;
    }
    ...
    //清除结果
    this.clear = function () {
        allWidgetQueue[this.widgetId].push({ type: "clearQueryWidget" });
        doWidgetQueue(this.widgetId);
    }
...
//一维水模型
function YWSModeWidgetFun(riverServiceUrl,riverInfoArray, evenInfo, qskInfo, option) {
    this.type = "YWSModelWidget";
    this.id = this.type + "_" + getIdNum();
    this.isWidget = true;
    this.widgetId = "YWSModelWidget";
    this.loadInterface = "YWSModelWidget_Loaded";
    var widgetParam = new Object();
    this.widgetParam = widgetParam;

    widgetParam.riverInfoArray = riverInfoArray;
    widgetParam.evenInfo = evenInfo;
    widgetParam.qskInfo = qskInfo;
    widgetParam.riverServiceUrl = riverServiceUrl;

    if (evenInfo) {
        widgetParam.longitude = evenInfo.longitude;
        widgetParam.latitude = evenInfo.latitude;

        widgetParam.pointurl = evenInfo.pointurl;
    }
    if (qskInfo) {
        widgetParam.qskRiverCode = qskInfo.qskRiverCode;
        widgetParam.qskName = qskInfo.qskName;

        widgetParam.qskType = qskInfo.qskType;
        widgetParam.qskAdress = qskInfo.qskAdress;

        widgetParam.qskLongitude = qskInfo.qskLongitude;
        widgetParam.qskLatitude = qskInfo.qskLatitude;
        widgetParam.qskIcon = qskInfo.qskIcon; 
    }
    if (option) {
        widgetParam.colorAry = option.colorAry;
        widgetParam.zoomScale = option.zoomScale;
...
function RiverInfoParamFun(option) {
    this.pointurl = option.pointurl;
    this.id = option.id;
    this.riverCode = option.riverCode;
    this.name = option.name;
    this.layername = option.layername;
    this.extent = option.extent;
    this.partLength = option.partLength;
}
function EventInfoParamFun(option) {
    this.longitude = option.longitude;
    this.latitude = option.latitude;
    this.pointurl = option.pointurl;
}
function QskInfoParamFun(option) {
    this.qskRiverCode = option.qskRiverCode;
    this.qskName = option.qskName;
    this.qskType = option.qskType;
    this.qskAdress = option.qskAdress;
    this.qskLongitude = option.qskLongitude;
    this.qskLatitude = option.qskLatitude;
    this.qskIcon = option.qskIcon;
}
