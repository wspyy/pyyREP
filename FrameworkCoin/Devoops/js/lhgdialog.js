...
reSizeHdl = function() {
	...
	SetIFramePage = function() {
	    ...
	    if (opt.html) {
	        ...
	        innerDoc = '<iframe id="lhgfrm_' + opt.id + '" frameborder="0" src="' + opt.page + '" ' +
				       'scrolling="no" style="display:none;width:100%;height:100%;overflow:hidden;"><\/iframe>';
	    }
		    ...
		    '<div id="lhgdlg_', opt.id, '" class="lhgdialog_', opt.skin, '" style="width:', opt.width, 'px;height:', opt.height, 'px;">',
				...
				'<tr>',
					...
					'<td id="lhgdg_drag_', opt.id, '" class="lhgdg_top_', opt.skin, '">', opt.titleBar ?
						('<div class="lhgdg_title_icon_' + opt.skin + '">' + (opt.iconTitle ? '<div class="lhgdg_icon_' + opt.skin + '"></div>' : '<div style="width:16px; float:left; height:16px;margin-top:13px; border:0px solid #000000;"><img src="' + opt.icon + '" style="display:block;padding-left:2px;" width="16px" height="16px"/></div>') +
	...
	SetDialog = function(dg) {
	    ...
	    if (opt.xButton && opt.titleBar) {
	        ...
	        J(xbtnObj).bind('click', S.fun_cancel || S.cancel);
	    }
	    ...
	    if (opt.minBtn) {
	        J(minBtnObj).bind('click', S.minSize);
	    }
	...
	removeDG = function() {
	    ...
	    if (J.isFunction(opt.onClose)) opt.onClose.call(S);
        ...
        S.ShowDialog = function() {
            ...
            if (opt.btnBar && opt.cancelBtn)
                S.addBtn('dgcancelBtn', opt.cancelBtnTxt, S.cancel);
        ...
        S.maxSize = function() {
            ...
            if (!maxed) {
                ...
                try {
                    //更新工具栏
                    MaxDialog(opt.param, cS.w, cS.h - 25, sS.y, sS.x);
                } catch (e)
                { }
            }
            else {
                ...
                try {
                    //更新工具栏
                    MaxDialog(opt.param, max.dgW, max.dgH, max.dgT, max.dgL);
                } catch (e)
                { }
