...
function MessagesMenuWidth(){
	...
	//alert(w_messages)
	//var W_menu1 = $('#logo').outerWidth();
	//var w_messages1 = (W - W_menu1) * 16.666666666666664 / 100;
	//alert(w_messages1);
...
(function ($) {
    //提示框
    $.AlertMess = function (title, mes) {
        if (title == null || title == ""){
            title = "提示";
        }
        var divMess = $('<div id="modalbox"><div class="devoops-modal"><div class="devoops-modal-header"><div class="modal-header-name"><span style="font-weight:bold;">' + title + '</span></div>' +
                   '<div class="box-icons"><a class="close-link"><i class="fa fa-times"></i></a></div></div><div class="devoops-modal-inner">' + mes + '</div>' +
                   '<div class="devoops-modal-bottom"><input  type="button" style="height:25px;" onclick="closeMes()" class="btn btn-info" value="确定"/></div></div></div>');
        $("body").append(divMess);
        divMess.fadeIn('fast');
        $('body').addClass("body-expanded");
    }
    //选择提示框
    $.ConfirmMess = function (title, mes,fun)
    {
        if (title == null || title == "") {
            title = "提示";
        }
        var divMess = $('<div id="modalbox"><div class="devoops-modal"><div class="devoops-modal-header"><div class="modal-header-name"><span style="font-weight:bold;">' + title + '</span></div>' +
                  '<div class="box-icons"><a class="close-link"><i class="fa fa-times"></i></a></div></div><div class="devoops-modal-inner">' + mes + '</div>' +
                  '<div class="devoops-modal-bottom"><input  type="button" style="height:25px;" onclick="confirmCallback(' + fun + ')" class="btn btn-info" value="确定"/><input  type="button" style="height:25px;margin-left:8px;" onclick="closeMes()" class="btn btn-info" value="取消"/></div></div></div>');
        $("body").append(divMess);
        divMess.fadeIn('fast');
        $('body').addClass("body-expanded");
    }
...
$(document).ready(function () {
    $('.show-sidebar').on('click', function (e) {
        ...
        var showsilder=$(this);
        if (showsilder.attr("attr") == "show") {
            showsilder.attr("attr", "hide");
            $("#logo").hide();
            $("#top-panel").removeClass("col-sm-10").addClass("col-sm-12");
        } else {
            showsilder.attr("attr", "show");
            $("#logo").show();
            $("#top-panel").removeClass("col-sm-12").addClass("col-sm-10");
        }
