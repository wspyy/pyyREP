   ...
   public class Pollution
    {
       ...
       public DataSet GetPollutionSourceList(string sqlWhere)
       {
           ...
           string sql = "select p.* ,e.[EnterpriseName],e.[IndustryName],e.[EnterpriseAddress],e.[DelFlag] from T_Mid_PollutionSources as p  inner join T_Bas_Enterprise as e on p.[EnterpriseCode]=e.[EnterpriseCode] where p.[EnterpriseCode]=" + "'" + sqlWhere + "'";
       ...
       {
           SQLHelper sqlh = new SQLHelper();
           string sql = @"SELECT *  FROM [dbo].[T_Bas_Enterprise] order by DelFlag,EnterpriseName";
           return sqlh.ExecuteSQLDataSet(sql);
       }
