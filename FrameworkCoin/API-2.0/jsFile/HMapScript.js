...
function addControlFun(control) {
    if (control.isWidget) {
        if (!widgetIsLoad[control.widgetId]) {            //判断模块是否加载
            ...
            allWidgetQueue[control.widgetId].push({ type: control.loadInterface, param: control.widgetParam }); //模块加载完成
...
//添加图形
function addGraphicFun(graphics) {
    var graLayerId = "graphicsLayer" + getIdNum();
    for (var i = 0; i < graphics.length; i++) {
        graphics[i] = getGraphicObj(graphics[i], graLayerId);
    }
    proQueue.push({ type: "addGraphic", param: { layerId: graLayerId, graphicList: graphics} });
    doProQueue();
    return graLayerId;
}
...
function addEventListenerFun(type, listener) {
    listenerArray.push({ id: "HMap", type: type, callback: listener });     //向监听列队中添加值
}
